...
import { foundItemAPI } from '../api/api';
...
export default function FoundItems() {
  ...
  const load = () => foundItemAPI.getAll().then((r) => setItems(r.data));
  ...
  const handleSearch = async (e) => {
    ...
    const res = await foundItemAPI.search(search);
    setItems(res.data);
  };

  const handleDelete = async (id) => {
    if (!confirm('Delete this item?')) return;
    await foundItemAPI.delete(id);
    load();
  };

  const handleSave = async (data) => {
    if (editing) {
      await foundItemAPI.update(editing.id, data);
    } else {
      await foundItemAPI.create({ ...data, userId: user?.id || user?.userId });
    }
    setShowForm(false);
    setEditing(null);
    load();
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <h2>Found Items</h2>
        <button style={styles.btn} onClick={() => { setEditing(null); setShowForm(true); }}>+ Report Found Item</button>
      </div>
        ...
        {items.map((item) => (
          <div key={item.id} style={styles.card}>
            ...
            <div style={styles.cardBody}>
              ...
              <span style={{ ...styles.badge, background: item.status === 'CLAIMED' ? '#34a853' : '#fbbc04' }}>{item.status}</span>
              <p style={styles.meta}><b>Category:</b> {item.category}</p>
              <p style={styles.meta}><b>Location Found:</b> {item.locationFound}</p>
              <p style={styles.meta}><b>Date:</b> {item.dateFound}</p>
      ...
      {items.length === 0 && <p style={{ color: '#888', marginTop: '24px' }}>No found items.</p>}
    </div>
  );
}
